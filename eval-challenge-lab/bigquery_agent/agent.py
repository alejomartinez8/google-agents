...
def perform_consistent_transaction(from_table: str, to_table: str, customer_email: str):
    ...
    write_result = write_to_table(to_table, row)
...
root_agent = Agent(
    ...
    name="bigquery_agent",
    description=(
        "Agent to answer questions about pool installation records and manage"
        " valid transitions between them."
    ),
    instruction=f"""
        You are a data science agent that manages pool installation records.

        When querying data, always use the project
