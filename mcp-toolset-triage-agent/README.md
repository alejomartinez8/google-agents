...
The simplest architecture of the four projects in this repo: one `Agent`, no sub-agents, no graph — contrast with [`graph-workflow-routing`](../graph-workflow-routing/)'s conditional-routing `Workflow` (deterministic auto-approve/policy-violation/LLM-review branches) or [`graph-workflow-fanout`](../graph-workflow-fanout/)'s fan-out/join graph. `McpToolset` + `StdioConnectionParams` spawns `cymbal_direct_mcp.py` as a local subprocess and talks to it over stdio — no external database, the "logistics system" is just in-memory Python dicts (5 orders, 5 customers spanning all 4 loyalty tiers, plus one already-delivered order as a negative case).
...
Unlike [`graph-workflow-fanout`](../graph-workflow-fanout/) and [`multi-agent-transfer-chain`](../multi-agent-transfer-chain/), most of this one was built by prompting the **Antigravity CLI (`agy`)** in natural language rather than writing code directly — that's the actual skill GENAI144 tests. Roughly, in order:
...
```bash
cd mcp-toolset-triage-agent
agents-cli deploy
```

## Notes

- **Runs more standalone than `graph-workflow-fanout`/`multi-agent-transfer-chain`** — no BigQuery table or Agent Search datastore to provision, since the "logistics system" is in-memory mock data inside `cymbal_direct_mcp.py` itself. You still need a GCP project (or a Gemini API key) for the model calls themselves — see `.env.example`.
