...
| [agent_platform_sandbox](agent_platform_sandbox/) | Cymbal Analytics Portfolio Analyst — demonstrates Agent Platform's Code Execution sandbox: computes portfolio risk metrics (returns, volatility, Sharpe ratio) and charts first via direct SDK calls, then via an ADK agent (`cymbal_analyst`) that autonomously generates and runs the same kind of Python against the same stateful sandbox. |
